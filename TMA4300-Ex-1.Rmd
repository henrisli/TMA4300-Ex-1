...
%```{r, echo = T, eval = T}
%histogram = hist(xsamples, breaks=20, probability=T, plot=F)

%x_val = seq(0, max(xsamples), length.out = 1000)

%plot(histogram$mids, histogram$density, ylim=c(0., 1.), col="blue", xlab = "x", ylab="P(x)")
%lines(x_val, f_A2(x_val, alpha), type='l', col="red")
%legend(x="topright", .95, legend=c("Analytic", "Inverse sampling"), col=c('red', 'blue'), %lty=c(1,NA),pch=c(NA,'o'), cex=0.8)
%```
```{r, echo = T, eval = T}
...
x = linspace(0, 10, 100)
y_gamma = dgamma(x, shape=3, rate=2)
y_prop = f_A2(x, 3)
...
%c = 10
%alpha = 1
%sample_gamma = function(size=1000){
%  uniform1 = runif(size)
%  uniform2 = runif(size)
%  x = F_inv(uniform1, alpha)
%  
%  print(as.matrix(f_A2(x, alpha)))
%  
%  probs = dgamma(x, 3, 2) * f_A2(x, alpha)
