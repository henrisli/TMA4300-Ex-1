...
#  z<- sapply(alpha,gamma)
#  x <- z/sum(z)
#  return(x)
#}
Gamma <- function(alpha){
  return(rgamma(1,alpha))
}
n = 10000

#alpha <- matrix(exponential(0.001,n),ncol = n, nrow = 1)
alpha <- matrix(rep(1,n),ncol = n, nrow = 1)
z <- sapply(alpha, Gamma)
x <- z/sum(z)
y <- as.vector(rdirichlet(1,alpha[1,]))
rfunc = data.frame(value = y)
ourfunc = data.frame(value = x)
rfunc$type = 'R function'
ourfunc$type = 'Our function'
...
#IMPLEMENTER VARIANS OG MEAN VS THEORETICAL
...
df2 <- data.frame(theo=seq(0,max(x),length.out=n), value=x)
h <- ggplot(df2, aes(x = value))
h <- h + geom_histogram(aes(y = ..density..), binwidth = 0.000005,fill = 'blue', color = 'blue') +stat_function(fun=dbeta,geom = "line",size=1.6,col="red",args = list(shape1=1,shape2=n-1))
h <- h + ggtitle("Our function vs. theoretical density")
h
#MARGINAL DENSITY OF DIRICHLET IS BETA DISTRIBUTION
```