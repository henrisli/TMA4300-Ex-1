...
We observe a slight shift in the IS estimate of the posterior mean when assuming a different prior. This is as expected, as such an estimate will fall between the Monte Carlo estimate under a uniform prior (`r cat(sum(y$samples)/n)`) and the expectation of the prior $B \sim Beta(1, 5) \Leftarrow E(B) = 1 / (1+5) = 0.167$. 

We investigate what proportion of its mass the $Beta(1, 5)$ prior has on the left side of `r cat(sum(y$samples)/n)`.
