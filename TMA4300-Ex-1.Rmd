...
```{r, echo = T, eval = T}
# The unscaled density
f_unscaled <- function(x){
  return((2+x)^125*(1-x)^38*x^34)
}

# Find the normalizing constant of f.
N_C = integrate(f_unscaled,0,1)$value 

#The normalized density. Corresponds to f in the text. 
f <- function(x){return(f_unscaled(x)/N_C)}
...
n = 10000
# The function x*f(x) is integrated from 0 to 1 to find the theoretical mean
g <- function(x){return(x*f(x))}

# Draw samples from the distribution f(theta|y)
y <- draw_samples(n) 

cat("Computed mean: ", sum(y$samples)/n, '\n')
cat("Theoretical mean: ", integrate(g,0,1)$value, '\n')

df <- data.frame(theo=seq(0,1,length.out=n), value=y$samples)

ggplot(df, aes(x = value)) + geom_histogram(aes(y = ..density.., colour = "Sampled"), binwidth=0.001) + stat_function(fun=f,geom = "line",size=1.6,aes(colour="Analytical")) + ggtitle("Comparison of analytical and sampled density") + xlim(0,1) + geom_segment(x = sum(y$samples)/n, xend = sum(y$samples)/n, y = 0, yend = 100, aes(colour = "Mean"), size = 1.2, linetype = "longdash") + labs(x='x', y='density', caption=paste('Comparison of the sampled density of f(theta|y) (blue bars) \n and the analytical density distribution (red line). \n The estimated posterior mean (green line) is indicated.'))
```
...
cat("Theoretical expected number: ", optimum$objective)
```
...
weights = posterior_target(y$samples) / f(y$samples)
...
We see that the IS posterior mean estimate is close to the analytical mean. The target and proposal densities are sufficiently close such that the bias introduced by estimating the new mean based on the already generated samples is sufficiently small. 
...
We compute the self-normalizing IS posterior mean estimate. 
```{r}
unscaled_weights = unscaled_posterior_target(y$samples) / f_unscaled(y$samples)
