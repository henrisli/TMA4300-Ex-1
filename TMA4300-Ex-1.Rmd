...
ggplot(df2, aes(x = value)) + geom_histogram(aes(y = ..density..), binwidth=0.1, col = 'blue', fill = 'blue') + stat_function(fun=f_A2,geom = "line",size=1.6,col="red",args = (mean=alpha)) + ggtitle("Comparison of analytical and sampled density")
```
...
small_gamma = function(alpha, n=1){
  ...
  while(sampled < n){
    ...
    probs = dgamma(x, alpha, rate=1) / (c * as.double(f_A2(x, alpha)))
...
x_val = seq(0, 5, lenght.out = 100)

df11 <- data.frame(x = x_val, y=dgamma(x_val, alpha, 1))

ggplot() + geom_histogram(data=df12, aes(x=samples, y=..density.., color="Sampled"), binwidth=0.1) + geom_line(data=df11, aes(x=x, y=y, color="Theoretical"), size=1.6) + ggtitle("Comparison of analytical and sampled density")
```
...
ggplot(df16, aes(x=data.samples)) + geom_histogram(data=df16, aes(x = data.samples, y=..density.., color="Sampled Function"), binwidth=0.1) + stat_function(fun=dgamma,geom = "line",size=1.6,col="red",args = list(shape=alpha, rate = 1))
```
...
ggplot(df15, aes(x = alpha, y= iterations, color="Iterations")) + geom_line()
```
