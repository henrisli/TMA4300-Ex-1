...
n_c <- integrate(f,0,1)
x_optim = optimize(f,c(0,1),maximum = TRUE)
#c = f(x_optim$maximum)/n_c$value
c = f(x_optim$maximum)
...
draw_samples <- function(n){
  ...
  for (i in 1:n){
    ...
    while(finish ==0){
      iter = iter + 1
      x_sample <- runif(1)
      #alpha <- 1/c*f(x_sample)/n_c$value
      alpha <- 1/c*f(x_sample)
      u = runif(1)
      if (u <=alpha){
        samples[i] <- x_sample
        finish <- 1
...
y <- draw_samples(100000)
sum(y$samples)/n
x_optim$maximum
cat("Theoretical mean: ", integrate(g,0,1)$value/n_c$value)
```
